...
@property(assign, nonatomic) BOOL hasFlashlight;
...
- (void)setCaptureSessionPreset:(NSString *)resolutionPreset {
  int presetIndex;
  if ([resolutionPreset isEqualToString:@"fhdx4"]) {
    presetIndex = 0;
  }else if ([resolutionPreset isEqualToString:@"fhd"]) {
    presetIndex = 1;
...
- (void)handleMethodCallAsync:(FlutterMethodCall *)call result:(FlutterResult)result {
  if ([@"availableCameras" isEqualToString:call.method]) {
    ...
    } else if ([@"lightOn" isEqualToString:call.method]) {
      NSNumber *intensity = call.arguments[@"intensity"];
      [self turnOnWithIntensity:intensity.doubleValue];
      result(nil);
    } else if ([@"lightOff" isEqualToString:call.method]) {
      [self turnOff];
      result(nil);
    } else if ([@"hasFlashlight" isEqualToString:call.method]) {
